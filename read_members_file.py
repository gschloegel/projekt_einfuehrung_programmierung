...
def calc_stats(taxids, all_taxids):
    ...
    # Your code here
    no_of_tax = len(set(taxids))
    no_of_unique_tax = len(uniques(taxids))
    total_no_of_tax = len(all_taxids)
    occurence = no_of_tax / total_no_of_tax * 100
    uniqueness = no_of_unique_tax / no_of_tax * 100
    occurence_as_singlecopy = no_of_unique_tax / total_no_of_tax * 100
...
def missing_taxids(cog, taxids):
    ...
    # Your code here
    for missing_tax_id in all_taxids - set(taxids):
        taxid2missing_cogs[missing_tax_id].append(cog)
...
def determine_cogs(args):
    ...
    for cog, taxids in cog2taxids.items():
        ...
        if (
            ...
            continue  # stopps current loop if conditions are not fulfilled

        missing_taxids(cog, taxids)
        # missing taxids is only executed for good cogs
        good_cogs.add(cog)
    ...
    if args.missing:
        ...
        # Your code here
        print(f"Organisems with more than {args.missing} missing COGs")
        # print(good_cogs)
        # print(taxid2missing_cogs)
        for taxid in taxid2missing_cogs:
            missing_good_cogs = taxid2missing_cogs[taxid]
            # taxid2missing_cog contains the missing cogs fullfilling the
            # other criteria (occurence, uniqueness, occurence_as_singlecopy)
            if len(missing_good_cogs) >= args.missing:
                print(f"Tax Id.: {taxid}")
                print(f"No. of missing COGs: {len(missing_good_cogs)}")
                print(f"missing COGs: {str(missing_good_cogs)[1:-1]}\n")
